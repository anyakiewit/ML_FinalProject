...
def main():
    ...
    X_train_pos = np.array([normalized_doc_position(ex.get("word_index", 0), ex.get("doc_length", 1)) for ex in train_context_windows]).reshape(-1, 1)
    X_val_pos = np.array([normalized_doc_position(ex.get("word_index", 0), ex.get("doc_length", 1)) for ex in val_context_windows]).reshape(-1, 1)

    nb_model = train_naive_bayes_baseline(X_train_pos, y_train)
    ...
    stat_feature_names = [
        "word_length", "punctuation_count", "is_all_caps", 
        "is_first_capitalized"
    ]
    comb_feature_names = ["mlm_surprisal"] + stat_feature_names
    ...
    plot_feature_importances(nb_model, feature_names=["norm_doc_pos"], title="Baseline Naive Bayes feature importance")
